...
KAFKA_BROKER = "localhost:9092"          # if running locally
# KAFKA_BROKER = "kafka:29092"           # if running inside Docker container

ES_INDEX = "youtube_data"
ES_HOST = "http://localhost:9200"        # local access
# ES_HOST = "http://elasticsearch:9200"  # for Docker network access
...
def connect_elasticsearch(retries=10, delay=5):
    for i in range(retries):
        try:
            ...
            print(f"⚠️  Elasticsearch not ready yet: {e}")
        time.sleep(delay)
